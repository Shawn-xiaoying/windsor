...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_WATER_SWITCH_STATE_2, DP_TYPE_BOOL},
  {DPID_WATER_FLOW, DP_TYPE_VALUE},
  {DPID_POWER_SWITCH, DP_TYPE_BOOL},
  {DPID_TEMP_SET_F, DP_TYPE_VALUE},
  {DPID_SOUND_SWICH, DP_TYPE_BOOL},
  {DPID_TEMP_UINT, DP_TYPE_ENUM},
  {DPID_OZONE_TIME, DP_TYPE_VALUE},
  {DPID_POWER_ON_ENABLE, DP_TYPE_BOOL},
  {DPID_WATER_SWITCH_STATE, DP_TYPE_BOOL},
...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_WATER_SWITCH_STATE_2,当前水流传感器2); //BOOL型数据上报;
    mcu_dp_value_update(DPID_WATER_FLOW,当前水流量); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_POWER_SWITCH,当前电源开关); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_SET_F,当前目标温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SOUND_SWICH,当前声音); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_TEMP_UINT,当前温度单位); //枚举型数据上报;
    mcu_dp_value_update(DPID_OZONE_TIME,当前臭氧阀开启时间); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_POWER_ON_ENABLE,当前上电开机); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_WATER_SWITCH_STATE,当前水流传感器); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_CURRENT_F,当前当前温度); //VALUE型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_power_switch_handle
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_power_switch_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
...
/*****************************************************************************
函数名称 : dp_download_sound_swich_handle
功能描述 : 针对DPID_SOUND_SWICH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sound_swich_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char sound_swich;
    
    sound_swich = mcu_get_dp_download_bool(value,length);
    if(sound_swich == 0) {
        //bool off
    }else {
        //bool on
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_SOUND_SWICH,sound_swich);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_temp_uint_handle
功能描述 : 针对DPID_TEMP_UINT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_temp_uint_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char temp_uint;
    
    temp_uint = mcu_get_dp_download_enum(value,length);
    switch(temp_uint) {
        case 0:
        break;
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_TEMP_UINT, temp_uint);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_ozone_time_handle
功能描述 : 针对DPID_OZONE_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_ozone_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long ozone_time;
    
    ozone_time = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_OZONE_TIME,ozone_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_power_on_enable_handle
功能描述 : 针对DPID_POWER_ON_ENABLE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_power_on_enable_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char power_on_enable;
    
    power_on_enable = mcu_get_dp_download_bool(value,length);
    if(power_on_enable == 0) {
        //bool off
    }else {
        //bool on
    }
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_POWER_ON_ENABLE,power_on_enable);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_POWER_SWITCH:
            //电源开关处理函数
            ret = dp_download_power_switch_handle(value,length);
        break;
        case DPID_TEMP_SET_F:
            //目标温度处理函数
            ret = dp_download_temp_set_f_handle(value,length);
        break;
        case DPID_SOUND_SWICH:
            //声音处理函数
            ret = dp_download_sound_swich_handle(value,length);
        break;
        case DPID_TEMP_UINT:
            //温度单位处理函数
            ret = dp_download_temp_uint_handle(value,length);
        break;
        case DPID_OZONE_TIME:
            //臭氧阀开启时间处理函数
            ret = dp_download_ozone_time_handle(value,length);
        break;
        case DPID_POWER_ON_ENABLE:
            //上电开机处理函数
            ret = dp_download_power_on_enable_handle(value,length);
        break;
