	...
	switch (evt)
    {
        ...
        case EFSM_EVT_ENTRY:
            ...
            app_set_cmd_send_task->timeCount = 0;
				    ...
				    if(app_set_cmd_send_task->set_feature_id_index <Set_Feature_Id_list.update_feature_num)
						{
								feature_id = Set_Feature_Id_list.feature_id[app_set_cmd_send_task->set_feature_id_index++] ;
								if(feature_id ==DEVICE_CODE_FEATURE_ID ) 
								{
									if(fstorge_write_device_code(SettingData.device_code)==false ) 
									EFSM_TRANS(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_ack_error);
								 
								}
								else {
								#ifdef USE_STATE_MACHINE
								set_cmd_send_type = APP_SET_CMD ;
								start_tuya_set_data_06cmd(feature_id);
						...
						NRF_LOG_INFO("start_dpid_set DPID_SET_ACK_OK end ");
						EFSM_TRANS(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_ack);
	...
	switch (evt)
    {
        ...
        case EFSM_EVT_TICK:
            ++app_set_cmd_send_task->globalTick;
						...
						EFSM_TRANS(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_ack_timeout);
				
            break;
    }
}
void app_set_cmd_send_task_ack_timeout(app_set_cmd_send_task_t *app_set_cmd_send_task, uint16_t evt)
{
	bluetooth_connection_status_task_t *bluetooth_connection_ownership_task;
	switch (evt)
    {
        case EFSM_EVT_EXIT:
						NRF_LOG_INFO("app_set_cmd_send_task_timeout exit =%d",app_set_cmd_send_task->globalTick);
            break;
        case EFSM_EVT_ENTRY:
						NRF_LOG_INFO("app_set_cmd_send_task_timeout entry =%d",app_set_cmd_send_task->globalTick);
            app_set_cmd_send_task->timeCount = 0;
...
void app_set_cmd_send_task_ack(app_set_cmd_send_task_t *app_set_cmd_send_task, uint16_t evt)
{
	 bluetooth_connection_status_task_t *bluetooth_connection_ownership_task;
	switch (evt)
    {
        case EFSM_EVT_EXIT:
						NRF_LOG_INFO("app_set_cmd_send_task_ack exit =%d",app_set_cmd_send_task->globalTick);
            break;
        case EFSM_EVT_ENTRY:
						NRF_LOG_INFO("app_set_cmd_send_task_ack entry =%d",app_set_cmd_send_task->globalTick);
            app_set_cmd_send_task->timeCount = 0;
           	Set_Feature_Id_list.update_feature_num = 0;
				    bluetooth_connection_ownership_task = find_ownership_bluetooth_connection_task(device_infor.ShortUserID);
						if (bluetooth_connection_ownership_task != NULL)
...
void app_set_cmd_send_task_ack_error(app_set_cmd_send_task_t *app_set_cmd_send_task, uint16_t evt)
{
	bluetooth_connection_status_task_t *bluetooth_connection_ownership_task;
	switch (evt)
    {
        case EFSM_EVT_EXIT:
						NRF_LOG_INFO("app_set_cmd_send_task_ack_error exit =%d",app_set_cmd_send_task->globalTick);
            break;
        case EFSM_EVT_ENTRY:
						NRF_LOG_INFO("app_set_cmd_send_task_ack_error entry =%d",app_set_cmd_send_task->globalTick);
            app_set_cmd_send_task->timeCount = 0;
				    Set_Feature_Id_list.update_feature_num = 0;
						bluetooth_connection_ownership_task = find_ownership_bluetooth_connection_task(device_infor.ShortUserID);
						if (bluetooth_connection_ownership_task != NULL)
...
void app_set_cmd_send_task_ctor(app_set_cmd_send_task_t *app_set_cmd_send_task)
{
    ...
    EFSM_REG_STATE(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_start);
	  ...
	  EFSM_REG_STATE(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_ack_timeout);
	  EFSM_REG_STATE(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_ack);
	  EFSM_REG_STATE(APP_SET_DATA_CMD_TASK_OBJ,app_set_cmd_send_task_ack_error);
