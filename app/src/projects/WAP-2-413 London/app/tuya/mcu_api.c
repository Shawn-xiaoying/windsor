...
/**
 * @brief  将2字节合并为1个16 bit变量
 * @param[in] {value} 2字节数组
 * @return 合并完成后的16bit变量
 */
unsigned short byte_to_short(const unsigned char value[2])
{
    unsigned short nubmer = 0;

    nubmer = (unsigned short)value[0];
    nubmer <<= 8;
    nubmer |= (unsigned short)value[1];
 ...
 * @note   Null
 */
unsigned char send_dp_raw_to_mcu(unsigned short dpid, const unsigned char value[], unsigned short len)
{
    unsigned short send_len = 0;

    //
    send_len = set_bluetooth_uart_byte(send_len,dpid/0x100);
		send_len = set_bluetooth_uart_byte(send_len,dpid% 0x100);
 ...
 * @note   Null
 */
unsigned char send_dp_bool_to_mcu(unsigned short dpid, unsigned char value)
{
    unsigned short send_len = 0;
 ...
 * @note   Null
 */
unsigned char send_dp_value_to_mcu(unsigned short dpid, unsigned long value)
{
    unsigned short send_len = 0;
 ...
 * @note   Null
 */
unsigned char send_dp_string_to_mcu(unsigned short dpid, const unsigned char value[], unsigned short len)
{
    unsigned short send_len = 0;
 ...
 * @note   Null
 */
unsigned char send_dp_enum_to_mcu(unsigned short dpid, unsigned char value)
{
    unsigned short send_len = 0;
 ...
 * @note   Null
 */
unsigned char send_dp_fault_to_mcu(unsigned short dpid, unsigned long value)
{
    unsigned short send_len = 0;
 ...
 * @note   Null
 */
unsigned char mcu_dp_enum_update(unsigned short dpid, unsigned char value)
{
#ifdef DEBUG_DATA_TO_APP
    char string[25] = {0};
    sprintf(string, "%d", value);
...
//#ifdef MCU_API_DEBUG_OUT
	  if(data_len<=80)
    {
			...
			for(j=0;j<data_len/80;j++)
			NRF_LOG_HEXDUMP_INFO(&value[j*80], 80);
			
			NRF_LOG_HEXDUMP_INFO(&value[j*80], data_len%80);
