...
static void mt_gpio_init(void)
{
   ...
   nrf_gpio_pin_set(RUN_LED_PIN);
	 ...
	 nrf_gpio_cfg_default(STM32_BOOT0_PIN);
   nrf_gpio_cfg_output(STM32_BOOT0_PIN);
	 nrf_gpio_pin_set(STM32_BOOT0_PIN);
 
	
	 nrf_gpio_cfg_default(STM32_RST_PIN);
   nrf_gpio_cfg_output(STM32_RST_PIN);
   nrf_gpio_pin_clear(STM32_RST_PIN);
	// nrf_delay_ms(100);
	// nrf_gpio_pin_set(STM32_RST_PIN);
